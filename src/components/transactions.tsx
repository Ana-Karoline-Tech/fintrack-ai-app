import Link from 'next/link'
import { AddTransactionButton } from '@/src/app/_components/add-transaction-button'

export type TransactionRow = {
    id: string
    title: string
    subtitle: string
    amount: number
    /** Emoji no quadrado colorido (Figma node 2:696) */
    iconEmoji: string
    iconBgClass: string
}

const MOCK_TRANSACTIONS: TransactionRow[] = [
    {
        id: '1',
        title: 'almoço restaurante',
        subtitle: '30 de setembro • Alimentação',
        amount: -42,
        iconEmoji: '🍔',
        iconBgClass: 'bg-[rgba(249,115,22,0.1)]',
    },
    {
        id: '2',
        title: 'freela',
        subtitle: '30 de setembro • Trabalho',
        amount: 1000,
        iconEmoji: '📦',
        iconBgClass: 'bg-[rgba(59,130,246,0.1)]',
    },
    {
        id: '3',
        title: 'Assinatura Streaming',
        subtitle: '28 de setembro • Entretenimento',
        amount: -55.9,
        iconEmoji: '📺',
        iconBgClass: 'bg-[rgba(99,102,241,0.1)]',
    },
]

function formatBRL(value: number): string {
    const formatted = new Intl.NumberFormat('pt-BR', {
        style: 'currency',
        currency: 'BRL',
    }).format(Math.abs(value))
    const sign = value >= 0 ? '+' : '-'
    return `${sign}${formatted}`
}

export function Transactions() {
    return (
        <section className="flex w-full flex-col gap-6 font-sans">
            <div className="flex flex-row items-center justify-between gap-4">
                <h2 className="text-xl font-bold leading-7 text-[#F1F5F9]">
                    Transações recentes
                </h2>
                <AddTransactionButton />
            </div>

            <div className="overflow-hidden rounded-3xl border border-[#1E293B] bg-[#161B26]">
                {MOCK_TRANSACTIONS.map((tx, index) => (
                    <div
                        key={tx.id}
                        className={`flex flex-row items-center justify-between gap-4 px-5 py-5 ${
                            index > 0 ? 'border-t border-[#1E293B]' : ''
                        }`}
                    >
                        <div className="flex min-w-0 flex-row items-center gap-4">
                            <div
                                className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl ${
                                    tx.iconBgClass
                                } ${index === 0 ? 'border border-black' : ''}`}
                                aria-hidden
                            >
                                <span className="text-[30px] leading-none">
                                    {tx.iconEmoji}
                                </span>
                            </div>
                            <div className="flex min-w-0 flex-col gap-0.5">
                                <p className="text-base font-bold leading-6 text-[#F1F5F9]">
                                    {tx.title}
                                </p>
                                <p className="text-xs font-normal leading-[1.33] text-[#64748B]">
                                    {tx.subtitle}
                                </p>
                            </div>
                        </div>
                        <p
                            className={`shrink-0 text-lg font-bold leading-7 ${
                                tx.amount >= 0
                                    ? 'text-[#10B981]'
                                    : 'text-[#F43F5E]'
                            }`}
                        >
                            {formatBRL(tx.amount)}
                        </p>
                    </div>
                ))}

                <div className="flex w-full justify-center bg-[rgba(30,41,59,0.2)] px-4 pb-[17px] pt-[18.5px]">
                    <Link
                        href="/transactions"
                        className="text-center text-sm font-semibold leading-5 text-[#9333EA] transition hover:text-[#a855f7]"
                    >
                        Ver todo o histórico
                    </Link>
                </div>
            </div>
        </section>
    )
}
